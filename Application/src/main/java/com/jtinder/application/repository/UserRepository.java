...
public interface UserRepository extends CrudRepository<User, Long> {
    ...
    @Query(value = "select * from usr\n" +
            ...
            "  and user_id != :userId\n" +
            "  and user_id not in (select like_id from user_relationships where user_id = :userId)\n" +
            "  and user_id in (select user_id from find_sex where find_sex = :sex)", nativeQuery = true)
